...
HAL_StatusTypeDef HAL_DMAEx_List_ConvertQToDynamic(DMA_QListTypeDef *const pQList)
{
  ...
  for (node_count = 1U; node_count < pQList->NodeNumber; node_count++)
  {
    /* Update node address */
                              ...
                              DMA_NodeTypeDef *const pDestNode)
{
  ...
  for (reg_idx = 0U; reg_idx < NODE_CLLR_IDX_POS; reg_idx++)
  {
                                ...
                                uint32_t Format)
{
  ...
  if (Format == NODE_DYNAMIC_FORMAT)
  {
    ...
    for (reg_idx = RegisterIdx; reg_idx < (RegisterNumber - 1U); reg_idx++)
    {
  ...
  {
    /* Repeat for all registers to be formatted */
    for (reg_idx = (RegisterNumber - 2U); reg_idx > RegisterIdx; reg_idx--)
    {
      pNode->LinkRegisters[reg_idx] = pNode->LinkRegisters[reg_idx - 1U];
    }
  }
                                       ...
                                       uint32_t FirstUnusedField)
{
  ...
  for (reg_idx = FirstUnusedField; reg_idx < NODE_CLLR_IDX_POS; reg_idx++)
  {
