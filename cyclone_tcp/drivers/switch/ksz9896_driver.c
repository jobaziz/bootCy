   ...
   const SwitchFdbEntry *entry)
{
   ...
   if(j < KSZ9896_STATIC_MAC_TABLE_SIZE)
   {
      ...
      value = (j << 16) & KSZ9896_STATIC_RES_MCAST_TABLE_CTRL_TABLE_INDEX;
      //Set the TABLE_SELECT bit to 0 to select the static address table
      value &= ~KSZ9896_STATIC_RES_MCAST_TABLE_CTRL_TABLE_SELECT;
      //Set the ACTION bit to 0 to indicate a write operation
      value &= ~KSZ9896_STATIC_RES_MCAST_TABLE_CTRL_ACTION;
      //Set the START_FINISH bit to 1 to initiate the operation
      value |= KSZ9896_STATIC_RES_MCAST_TABLE_CTRL_START_FINISH;

      //Start the write operation
      ksz9896WriteSwitchReg32(interface, KSZ9896_STATIC_RES_MCAST_TABLE_CTRL,
         value);
      ...
      do
      {
         ...
         value = ksz9896ReadSwitchReg32(interface,
            KSZ9896_STATIC_RES_MCAST_TABLE_CTRL);

         //Poll the START_FINISH bit
      } while((value & KSZ9896_STATIC_RES_MCAST_TABLE_CTRL_START_FINISH) != 0);
   ...
   SwitchFdbEntry *entry)
{
   ...
   {
      //Write the TABLE_INDEX field with the 4-bit index value
      value = (index << 16) & KSZ9896_STATIC_RES_MCAST_TABLE_CTRL_TABLE_INDEX;
      //Set the TABLE_SELECT bit to 0 to select the static address table
      value &= ~KSZ9896_STATIC_RES_MCAST_TABLE_CTRL_TABLE_SELECT;
...
void ksz9896FlushStaticFdbTable(NetInterface *interface)
{
   ...
   for(i = 0; i < KSZ9896_STATIC_MAC_TABLE_SIZE; i++)
   {
      ...
      //Write the TABLE_INDEX field with the 4-bit index value
      value = (i << 16) & KSZ9896_STATIC_RES_MCAST_TABLE_CTRL_TABLE_INDEX;
      //Set the TABLE_SELECT bit to 0 to select the static address table
      value &= ~KSZ9896_STATIC_RES_MCAST_TABLE_CTRL_TABLE_SELECT;
      //Set the ACTION bit to 0 to indicate a write operation
      value &= ~KSZ9896_STATIC_RES_MCAST_TABLE_CTRL_ACTION;
