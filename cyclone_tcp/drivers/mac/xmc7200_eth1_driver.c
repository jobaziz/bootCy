/**
 * @file xmc7200_eth1_driver.c
 * @brief XMC7200 Ethernet MAC driver (ETH0 instance)
 *
 * @section License
 *
...
#include "cy_device.h"
#include "core/net.h"
#include "drivers/mac/xmc7200_eth1_driver.h"
#include "debug.h"
...
#pragma location = XMC7200_ETH1_RAM_SECTION
static uint8_t txBuffer[XMC7200_ETH1_TX_BUFFER_COUNT][XMC7200_ETH1_TX_BUFFER_SIZE];
//RX buffer
#pragma data_alignment = 8
#pragma location = XMC7200_ETH1_RAM_SECTION
static uint8_t rxBuffer[XMC7200_ETH1_RX_BUFFER_COUNT][XMC7200_ETH1_RX_BUFFER_SIZE];
//TX buffer descriptors
#pragma data_alignment = 4
#pragma location = XMC7200_ETH1_RAM_SECTION
static Xmc7200Eth1TxBufferDesc txBufferDesc[XMC7200_ETH1_TX_BUFFER_COUNT];
//RX buffer descriptors
#pragma data_alignment = 4
#pragma location = XMC7200_ETH1_RAM_SECTION
static Xmc7200Eth1RxBufferDesc rxBufferDesc[XMC7200_ETH1_RX_BUFFER_COUNT];

//Dummy TX buffer
#pragma data_alignment = 8
#pragma location = XMC7200_ETH1_RAM_SECTION
static uint8_t dummyTxBuffer[XMC7200_ETH1_DUMMY_BUFFER_COUNT][XMC7200_ETH1_DUMMY_BUFFER_SIZE];
//Dummy RX buffer
#pragma data_alignment = 8
#pragma location = XMC7200_ETH1_RAM_SECTION
static uint8_t dummyRxBuffer[XMC7200_ETH1_DUMMY_BUFFER_COUNT][XMC7200_ETH1_DUMMY_BUFFER_SIZE];
//Dummy TX buffer descriptors
#pragma data_alignment = 4
#pragma location = XMC7200_ETH1_RAM_SECTION
static Xmc7200Eth1TxBufferDesc dummyTxBufferDesc[XMC7200_ETH1_DUMMY_BUFFER_COUNT];
//Dummy RX buffer descriptors
#pragma data_alignment = 4
#pragma location = XMC7200_ETH1_RAM_SECTION
static Xmc7200Eth1RxBufferDesc dummyRxBufferDesc[XMC7200_ETH1_DUMMY_BUFFER_COUNT];
...
//TX buffer
static uint8_t txBuffer[XMC7200_ETH1_TX_BUFFER_COUNT][XMC7200_ETH1_TX_BUFFER_SIZE]
   __attribute__((aligned(8), __section__(XMC7200_ETH1_RAM_SECTION)));
//RX buffer
static uint8_t rxBuffer[XMC7200_ETH1_RX_BUFFER_COUNT][XMC7200_ETH1_RX_BUFFER_SIZE]
   __attribute__((aligned(8), __section__(XMC7200_ETH1_RAM_SECTION)));
//TX buffer descriptors
static Xmc7200Eth1TxBufferDesc txBufferDesc[XMC7200_ETH1_TX_BUFFER_COUNT]
   __attribute__((aligned(4), __section__(XMC7200_ETH1_RAM_SECTION)));
//RX buffer descriptors
static Xmc7200Eth1RxBufferDesc rxBufferDesc[XMC7200_ETH1_RX_BUFFER_COUNT]
   __attribute__((aligned(4), __section__(XMC7200_ETH1_RAM_SECTION)));
...
const NicDriver xmc7200Eth1Driver =
{
   NIC_TYPE_ETHERNET,
   ETH_MTU,
   xmc7200Eth1Init,
   xmc7200Eth1Tick,
   xmc7200Eth1EnableIrq,
   xmc7200Eth1DisableIrq,
   xmc7200Eth1EventHandler,
   xmc7200Eth1SendPacket,
   xmc7200Eth1UpdateMacAddrFilter,
   xmc7200Eth1UpdateMacConfig,
   xmc7200Eth1WritePhyReg,
   xmc7200Eth1ReadPhyReg,
   TRUE,
   TRUE,
   TRUE,
   FALSE
...
error_t xmc7200Eth1Init(NetInterface *interface)
{
   error_t error;
   volatile uint32_t status;

   //Debug message
   TRACE_INFO("Initializing XMC7200 Ethernet MAC (ETH0)...\r\n");
   ...
   ETH0->NETWORK_CONTROL = 0;

   //GPIO configuration
   xmc7200Eth1InitGpio(interface);

   //Configure MDC clock speed
   ETH0->NETWORK_CONFIG = 7 << ETH_NETWORK_CONFIG_MDC_CLOCK_DIVISION_Pos;
   //Enable management port (MDC and MDIO)
   ETH0->NETWORK_CONTROL |= ETH_NETWORK_CONTROL_MAN_PORT_EN_Msk;
   ...
   ETH0->SPEC_ADD1_BOTTOM = interface->macAddr.w[0] | (interface->macAddr.w[1] << 16);
   ETH0->SPEC_ADD1_TOP = interface->macAddr.w[2];

   //The MAC supports 3 additional addresses for unicast perfect filtering
   ETH0->SPEC_ADD2_BOTTOM = 0;
   ETH0->SPEC_ADD3_BOTTOM = 0;
   ETH0->SPEC_ADD4_BOTTOM = 0;
   ...
   ETH0->NETWORK_CONFIG |= ETH_NETWORK_CONFIG_RECEIVE_1536_BYTE_FRAMES_Msk |
      ETH_NETWORK_CONFIG_MULTICAST_HASH_ENABLE_Msk;

   //DMA configuration
   ETH0->GMAC_DCFGR = GMAC_DCFGR_DRBS(XMC7200_ETH1_RX_BUFFER_SIZE / 64) |
      GMAC_DCFGR_TXPBMS_Msk | GMAC_DCFGR_RXBMS_FULL | GMAC_DCFGR_FBLDO_INCR4;

   ETH0->DMA_RXBUF_SIZE_Q1 = XMC7200_ETH1_DUMMY_BUFFER_SIZE / 64;
   ETH0->DMA_RXBUF_SIZE_Q2 = XMC7200_ETH1_DUMMY_BUFFER_SIZE / 64;

   //Initialize buffer descriptors
   xmc7200Eth1InitBufferDesc(interface);

   //Clear transmit status register
   ETH0->TRANSMIT_STATUS = ETH_TRANSMIT_STATUS_RESP_NOT_OK123_Msk |
   ...
   ETH0->RECEIVE_STATUS = ETH_RECEIVE_STATUS_RESP_NOT_OK1234_Msk |
   ...
   ETH0->INT_DISABLE = 0xFFFFFFFF;
   ETH0->INT_Q1_DISABLE = 0xFFFFFFFF;
   ETH0->INT_Q2_DISABLE = 0xFFFFFFFF;

   //Only the desired ones are enabled
   ETH0->INT_ENABLE = ETH_INT_ENABLE_ENABLE_RESP_NOT_OK_INTERRUPT_Msk |
   ...
   status = ETH0->INT_STATUS;
   status = ETH0->INT_Q1_STATUS;
   status = ETH0->INT_Q2_STATUS;
   (void) status;

   //Set priority grouping (3 bits for pre-emption priority, no bits for subpriority)
   NVIC_SetPriorityGrouping(XMC7200_ETH1_IRQ_PRIORITY_GROUPING);

   //Configure GMAC interrupt priority
   NVIC_SetPriority(GMAC_IRQn, NVIC_EncodePriority(XMC7200_ETH1_IRQ_PRIORITY_GROUPING,
      XMC7200_ETH1_IRQ_GROUP_PRIORITY, XMC7200_ETH1_IRQ_SUB_PRIORITY));

   //Enable the GMAC to transmit and receive data
   ETH0->NETWORK_CONTROL |= ETH_NETWORK_CONTROL_ENABLE_TRANSMIT_Msk |
...
__weak_func void xmc7200Eth1InitGpio(NetInterface *interface)
{
...
void xmc7200Eth1InitBufferDesc(NetInterface *interface)
{
   uint_t i;
   uint32_t address;

   //Initialize TX buffer descriptors
   for(i = 0; i < XMC7200_ETH1_TX_BUFFER_COUNT; i++)
   {
   ...
   for(i = 0; i < XMC7200_ETH1_RX_BUFFER_COUNT; i++)
   {
   ...
   for(i = 0; i < XMC7200_ETH1_DUMMY_BUFFER_COUNT; i++)
   {
      //Calculate the address of the current TX buffer
   ...
   ETH0->TRANSMIT_Q_PTR = (uint32_t) txBufferDesc;
   ETH0->TRANSMIT_Q1_PTR = (uint32_t) dummyTxBufferDesc;
   ETH0->TRANSMIT_Q2_PTR = (uint32_t) dummyTxBufferDesc;

   //Start location of the RX descriptor list
   ETH0->RECEIVE_Q_PTR = (uint32_t) rxBufferDesc;
   ETH0->RECEIVE_Q1_PTR = (uint32_t) dummyRxBufferDesc;
   ETH0->RECEIVE_Q2_PTR = (uint32_t) dummyRxBufferDesc;
}
...
void xmc7200Eth1Tick(NetInterface *interface)
{
   //Valid Ethernet PHY or switch driver?
   if(interface->phyDriver != NULL)
   {
...
void xmc7200Eth1EnableIrq(NetInterface *interface)
{
...
void xmc7200Eth1DisableIrq(NetInterface *interface)
{
...
void GMAC_Handler(void)
{
   ...
   isr = ETH0->INT_Q1_STATUS;
   isr = ETH0->INT_Q2_STATUS;
   isr = ETH0->INT_STATUS;
   tsr = ETH0->TRANSMIT_STATUS;
   rsr = ETH0->RECEIVE_STATUS;
   (void) isr;
...
void xmc7200Eth1EventHandler(NetInterface *interface)
{
   error_t error;
   uint32_t rsr;
      ...
      ETH_RECEIVE_STATUS_BUFFER_NOT_AVAILABLE_Msk)) != 0)
   {
      ...
      ETH0->RECEIVE_STATUS = rsr;

      //Process all pending packets
      do
      {
         //Read incoming packet
         error = xmc7200Eth1ReceivePacket(interface);
...
error_t xmc7200Eth1SendPacket(NetInterface *interface,
   const NetBuffer *buffer, size_t offset, NetTxAncillary *ancillary)
{
   static uint32_t temp[XMC7200_ETH1_TX_BUFFER_SIZE / 4];
   size_t length;
   ...
   if(length > XMC7200_ETH1_TX_BUFFER_SIZE)
   {
   ...
   if(txBufferIndex < (XMC7200_ETH1_TX_BUFFER_COUNT - 1))
   {
   ...
   ETH0->NETWORK_CONTROL |= ETH_NETWORK_CONTROL_TX_START_PCLK_Msk;
...
error_t xmc7200Eth1ReceivePacket(NetInterface *interface)
{
   ...
   for(i = 0; i < XMC7200_ETH1_RX_BUFFER_COUNT; i++)
   {
      ...
      if(j >= XMC7200_ETH1_RX_BUFFER_COUNT)
      {
         j -= XMC7200_ETH1_RX_BUFFER_COUNT;
      }
   ...
   for(i = 0; i < j; i++)
   {
      ...
      if(eofIndex != UINT_MAX && i >= sofIndex && i <= eofIndex)
      {
         ...
         n = MIN(size, XMC7200_ETH1_RX_BUFFER_SIZE);
      ...
      //Wrap around to the beginning of the buffer if necessary
      if(rxBufferIndex >= XMC7200_ETH1_RX_BUFFER_COUNT)
      {
         rxBufferIndex = 0;
      }
   }
...
error_t xmc7200Eth1UpdateMacAddrFilter(NetInterface *interface)
{
   uint_t i;
   uint_t j;
   uint_t k;
   ...
   if(j >= 1)
   {
      ...
      ETH0->SPEC_ADD2_BOTTOM = unicastMacAddr[0].w[0] | (unicastMacAddr[0].w[1] << 16);
      ETH0->SPEC_ADD2_TOP = unicastMacAddr[0].w[2];
   }
   else
   {
   ...
   {
      //The address is activated when SAT register is written
      ETH0->SPEC_ADD3_BOTTOM = unicastMacAddr[1].w[0] | (unicastMacAddr[1].w[1] << 16);
      ETH0->SPEC_ADD3_TOP = unicastMacAddr[1].w[2];
   }
   else
   {
      //The address is deactivated when SAB register is written
   ...
   {
      //The address is activated when SAT register is written
      ETH0->SPEC_ADD4_BOTTOM = unicastMacAddr[2].w[0] | (unicastMacAddr[2].w[1] << 16);
      ETH0->SPEC_ADD4_TOP = unicastMacAddr[2].w[2];
   }
   else
   {
      //The address is deactivated when SAB register is written
   ...
   if(j >= 4)
   {
      ETH0->NETWORK_CONFIG |= ETH_NETWORK_CONFIG_UNICAST_HASH_ENABLE_Msk;
   }
   else
   {
      ETH0->NETWORK_CONFIG &= ~ETH_NETWORK_CONFIG_UNICAST_HASH_ENABLE_Msk;
   ...
   ETH0->HASH_BOTTOM = hashTable[0];
   ETH0->HASH_TOP = hashTable[1];
...
error_t xmc7200Eth1UpdateMacConfig(NetInterface *interface)
{
   uint32_t config;

   //Read network configuration register
   config = ETH0->NETWORK_CONFIG;
...
void xmc7200Eth1WritePhyReg(uint8_t opcode, uint8_t phyAddr,
   uint8_t regAddr, uint16_t data)
{
   ...
   if(opcode == SMI_OPCODE_WRITE)
   {
      ...
      ETH0->PHY_MANAGEMENT = temp;
      //Wait for the write to complete
      while((ETH0->NETWORK_STATUS & ETH_NETWORK_STATUS_MAN_DONE_Msk) == 0)
      {
      }
   }
   else
...
uint16_t xmc7200Eth1ReadPhyReg(uint8_t opcode, uint8_t phyAddr,
   uint8_t regAddr)
{
   uint16_t data;
   uint32_t temp;
   ...
   if(opcode == SMI_OPCODE_READ)
   {
      ...
      //Get register value
      data = ETH0->PHY_MANAGEMENT & ETH_PHY_MANAGEMENT_PHY_WRITE_READ_DATA_Msk;
   }
   else
   {
      //The MAC peripheral only supports standard Clause 22 opcodes
