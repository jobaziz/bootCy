   ...
   uint8_t *t, const uint8_t *p, uint8_t *c, size_t length)
{
   ...
   if(cipher == AES_CIPHER_ALGO)
   {
      ...
      if(m == (AES_BLOCK_SIZE * 8))
      {
         ...
         {
            //Encrypt payload data
            aesProcessData(context, t, p, c, length, CAU_MODE_AES_CTR |
               CAU_ENCRYPT);
         }
         else
         {
...
#endif
#if (GCM_SUPPORT == ENABLED && AES_SUPPORT == ENABLED && defined(CAU_MODE_AES_GCM))
...
#endif
#if (CCM_SUPPORT == ENABLED && AES_SUPPORT == ENABLED && defined(CAU_MODE_AES_CCM))
