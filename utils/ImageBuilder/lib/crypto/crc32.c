...
//CRC32 calculation lookup table
...
const HashAlgo crc32HashAlgo =
{
   ...
   0,
   FALSE,
   (HashAlgoCompute)crc32Compute,
   (HashAlgoInit)crc32Init,
   (HashAlgoUpdate)crc32Update,
   (HashAlgoFinal)crc32Final,
   (HashAlgoFinalRaw)NULL
};
...
/**
 * @brief Update the CRC32 context with a portion of the message being hashed
...
/**
 * @brief Finish the CRC32 message digest
 * @param[in] context Pointer to the CRC32 context
